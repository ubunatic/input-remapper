...
class Reader:
    ...
    def read(self):
        ...
        while self._results.poll():
            ...
            if event.value == 0:
                logger.processing_key(event_tuple, "release")
                self._release(type_code)
                continue

            if self._unreleased.get(type_code) == event_tuple:
                logger.processing_key(event_tuple, "duplicate key down")
            ...
            logger.processing_key(event_tuple, "down")
        ...
        if len(self._unreleased) > 0:
            ...
            logger.processing_key(result.keys, "read result")
