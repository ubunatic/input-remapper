...
def _parse_recurse(code, context, macro_instance=None, depth=0):
    ...
    if code.startswith('"'):
        ...
        string = code[1:-1]
        logger.processing("%sstring %s", space, string)
        return string
    ...
    if _is_number(code):
        if "." in code:
            ...
            code = int(code)
        logger.processing("%snumber %s", space, code)
        return code
    ...
    if call is not None:
        ...
        logger.processing("%scalls %s with %s", space, call, inner)
        ...
        if len(code) > position and code[position] == ".":
            ...
            logger.processing("%sfollowed by %s", space, chain)
    ...
    logger.processing("%sstring %s", space, code)
    return code
...
def parse(macro, context=None, return_errors=False):
    ...
    if return_errors:
        logger.processing("checking the syntax of %s", macro)
    else:
        logger.processing("preparing macro %s for later execution", macro)
