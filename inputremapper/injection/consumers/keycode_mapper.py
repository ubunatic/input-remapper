        ...
        if utils.is_key_down(action):
            ...
            for subset in subsets(combination):
                ...
                if len(combination) > 1:
                    logger.processing_key(combination, "unknown combination")
    ...
    def handle_keycode(self, event, action, forward=True):
        ...
        if utils.is_key_up(action):
            if active_macro is not None and active_macro.is_holding():
                ...
                logger.processing_key(key, "releasing macro")
            ...
            if type_and_code in unreleased:
                ...
                if target_code == DISABLE_CODE:
                    logger.processing_key(key, "releasing disabled key")
                    return

                if target_code is None:
                    logger.processing_key(key, "releasing key")
                    return
                ...
                if unreleased_entry.is_mapped():
                    ...
                    try:
                        ...
                        logger.processing_key(key, "could not map")
                        pass

                if forward:
                    # forward the release event
                    logger.processing_key((original_tuple,), "forwarding release")
                    self.forward(original_tuple)
                else:
                    logger.processing_key(key, "not forwarding release")
            ...
            if event.type != EV_ABS:
                ...
                logger.processing_key(key, "unexpected key up")
        ...
        if is_mapped and utils.is_key_down(action):
            ...
            if find_by_key(key) is not None:
                ...
                logger.processing_key(key, "duplicate key down")
                return
            ...
            if in_macros and running:
                ...
                logger.processing_key(key, "macro already running")
        ...
        if utils.is_key_down(action):
            ...
            if key in self.context.key_to_code:
                ...
                if target_code == DISABLE_CODE:
                    logger.processing_key(key, "disabled")
                    return
            ...
            if forward:
                logger.processing_key((original_tuple,), "forwarding")
                self.forward(original_tuple)
            else:
                logger.processing_key(((*type_and_code, action),), "not forwarding")
